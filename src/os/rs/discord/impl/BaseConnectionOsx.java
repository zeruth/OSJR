...
class BaseConnectionOsx extends BaseConnectionUnix {
	...
	private void registerUrl(String applicationId) {
		 ...
		 * CFStringRef cfURL = CFStringCreateWithCString(NULL, url,
		 * kCFStringEncodingUTF8);
		 * 
		 * NSString* myBundleId = [[NSBundle mainBundle] bundleIdentifier]; if
		 * (!myBundleId) { fprintf(stderr, "No bundle id found\n"); return; }
		 * 
		 * NSURL* myURL = [[NSBundle mainBundle] bundleURL]; if (!myURL) {
		 * fprintf(stderr, "No bundle url found\n"); return; }
		 * 
		 * OSStatus status = LSSetDefaultHandlerForURLScheme(cfURL, (__bridge
		 * CFStringRef)myBundleId); if (status != noErr) { fprintf(stderr,
		 * "Error in LSSetDefaultHandlerForURLScheme: %d\n", (int)status); return; }
		 * 
		 * status = LSRegisterURL((__bridge CFURLRef)myURL, true); if (status != noErr)
